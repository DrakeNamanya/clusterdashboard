...
// Table grouped by shg_name. KPIs: Unique Participants (Sales) /
//   New Participants (Sales) / Unique SHGs (Sales). Slicers: district + value_chain.
// Data from /api/sales (+ /options).
// ---------------------------------------------------------------------------

export function renderSales(base: string, opts: any = {}): string {
...
<head>
  ...
  <title>Sales in Horticulture/Oilseeds</title>
...
${navSidebar('sales')}
  <div class="max-w-[1600px] mx-auto p-3 md:p-4">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-3">
      ...
      <div class="card kpi px-4 py-2">
        <div class="text-[10px] uppercase tracking-wide text-[var(--muted)] font-semibold">New Participants (Sales)</div>
        <div id="kpiNew" class="text-2xl font-extrabold mt-0.5">–</div>
      </div>
      <div class="card kpi px-4 py-2">
        <div class="text-[10px] uppercase tracking-wide text-[var(--muted)] font-semibold">Unique SHGs (Sales)</div>
        <div id="kpiShgs" class="text-2xl font-extrabold mt-0.5">–</div>
      </div>

      <div class="ttl px-5 py-1.5 flex-1 text-center"><h1 class="text-lg md:text-2xl font-extrabold tracking-tight">SALES IN HORTICULTURE/OILSEEDS</h1></div>
  ...
  <script>
    ...
    const COLS = [
      ...
      { key:'qty_harvested',         label:'Sum of qty_harvested',        type:'num' },
      { key:'qty_harvested_measure', label:'First qty_harvested_measure', type:'txt' },
      { key:'total_planting_value',  label:'Sum of total_planting_value', type:'num' },
      { key:'net_planting',          label:'Sum of net_planting',         type:'num' },
    ...
    document.getElementById('refreshBtn').addEventListener('click', async (e)=>{
      ...
      try{ await fetch('/api/sales/refresh', {method:'POST'}); await load(); }
