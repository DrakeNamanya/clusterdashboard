...
//     pdn_level='marketing' AND value_chain='poultry'
//   Join: marketing[shg_participant_id] -> participants[refID]
//           -> shg_id, shg_name; participants[shg_id] -> shg_profiling_form -> Profilers_name
// Table grouped by shg_name. KPIs: Unique Participants / New Participants /
//   Unique SHGs. Slicers: district + poultry type + profilers_name.
// Data from /api/poultry-sales (+ /options).
// ---------------------------------------------------------------------------

export function renderPoultrySales(base: string, opts: any = {}): string {
  const bootOpts = JSON.stringify({
    districts: opts.districts || [],
    poultry_types: opts.poultry_types || [],
...
${navSidebar('poultrysales')}
  <div class="max-w-[1600px] mx-auto p-3 md:p-4">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-3">
      ...
      <div class="card kpi px-4 py-2">
        <div class="text-[10px] uppercase tracking-wide text-[var(--muted)] font-semibold">New Participants</div>
      ...
      <div class="ttl px-5 py-1.5 flex-1 text-center"><h1 class="text-lg md:text-2xl font-extrabold tracking-tight">POULTRY SALES</h1></div>
    ...
    <div class="grid grid-cols-12 gap-3">
      ...
      <aside class="col-span-12 lg:col-span-2 space-y-2">
        ...
        <div class="card p-2" id="sl-type"></div>
  ...
  <script>
    ...
    const COLS = [
      { key:'qty_produced',        label:'Sum of qty_produced',        type:'num' },
      { key:'poultry_sold',        label:'Sum of poultry_sold',        type:'num' },
      { key:'avg_bird_price',      label:'Sum of avg_bird_price',      type:'num' },
      { key:'total_poultry_value', label:'Sum of total_poultry_value', type:'num' },
      { key:'net_poultry',         label:'Sum of net_poultry',         type:'num' },
    ...
    const SL = [
      ...
      { id:'type', mount:'sl-type', label:'Poultry type',   optsKey:'poultry_types', search:true },
      { id:'prof', mount:'sl-prof', label:'Profile_name',   optsKey:'profilers',     search:true },
    ];
    const SL_PARAM = { dist:'districts', type:'poultry', prof:'profilers' };

    const S = {};
    SL.forEach(s => S[s.id] = { id:s.id, opts:[], sel:new Set(), all:true, cfg:s });

    let sortKey = 'total_poultry_value';
    let lastData = null;
    ...
    function buildSlicerShell(s){
      ...
      document.getElementById(s.cfg.mount).innerHTML =
        ...
        + '</div>'
        + '<div id="'+s.id+'List" class="scrollbar-thin overflow-y-auto max-h-[240px] pr-1"></div>';
      if (s.cfg.search)
    ...
    function renderTable(){
      ...
      const totalKeys = { qty_produced:1, poultry_sold:1, avg_bird_price:1, total_poultry_value:1, net_poultry:1 };
    ...
    async function loadOptions(){
      try{
        const res = await fetch('/api/poultry-sales/options');
    ...
    async function load(){
      ...
      try{
        const res = await fetch('/api/poultry-sales?'+params.toString());
        if (!res.ok) throw new Error('HTTP '+res.status);
        const d = await res.json();
        lastData = d;
    ...
    document.getElementById('refreshBtn').addEventListener('click', async (e)=>{
      ...
      try{ await fetch('/api/poultry-sales/refresh', {method:'POST'}); await load(); }
