...
//   Profilers_name / District_SHG = RELATED(shg_profiling_form[...])
// Table grouped by shg_name (sums), with First profiler / First District_SHG.
// KPI: SHG_Saving = DISTINCTCOUNT(isla_final[shg_id]) over the filtered rows.
// Slicers: District_SHG (list, incl. (Blank)), Profilers_name. Date range on
//   activity_date. Data from /api/isla (+ /options).
// ---------------------------------------------------------------------------

export function renderIsla(base: string, opts: any = {}): string {
...
<head>
  ...
  <title>SHGs Saving in a Cluster (ISLA)</title>
...
<body>
  <div class="max-w-[1600px] mx-auto p-3 md:p-4">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-3">
      ...
      <div class="card kpi px-4 py-2">
        <div class="text-[10px] uppercase tracking-wide text-[var(--muted)] font-semibold">SHG_Saving</div>
        <div id="kpiSaving" class="text-2xl font-extrabold mt-0.5">–</div>
      </div>

      <div class="ttl px-5 py-1.5 flex-1 text-center"><h1 class="text-lg md:text-2xl font-extrabold tracking-tight">SHGs SAVING IN A CLUSTER(ISLA)</h1></div>
  ...
  <script>
    ...
    const COLS = [
      { key:'savings_value',           label:'Sum of savings_value',           type:'num' },
      { key:'youth_group_saving',      label:'Sum of youth_group_saving',      type:'num' },
      { key:'youth_loans_value_given', label:'Sum of youth_loans_value_given', type:'num' },
      { key:'total_fund',              label:'Sum of total_fund',              type:'num' },
      { key:'loans',                   label:'Sum of loans',                   type:'num' },
      { key:'profilers_name',          label:'First Profilers_name',           type:'txt' },
      { key:'district_shg',            label:'First District_SHG',             type:'txt' },
    ];

    const SL = [
      { id:'dist', mount:'sl-dist', label:'District_SHG',    optsKey:'districts', search:true },
      { id:'prof', mount:'sl-prof', label:'Profilers_name',  optsKey:'profilers', search:true },
    ];
    ...
    async function loadOptions(){
      try{
        const res = await fetch('/api/isla/options');
    ...
    async function load(){
      ...
      try{
        const res = await fetch('/api/isla?'+params.toString());
        if (!res.ok) throw new Error('HTTP '+res.status);
        const d = await res.json();
        lastData = d;
        const ks = document.getElementById('kpiSaving'); if (ks) ks.textContent = fmt(d.shg_saving);
        for (const s of SL){
    ...
    document.getElementById('refreshBtn').addEventListener('click', async (e)=>{
      ...
      try{ await fetch('/api/isla/refresh', {method:'POST'}); await load(); }
